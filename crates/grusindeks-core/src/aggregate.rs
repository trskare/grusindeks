...
use crate::daily::{reason_for, Confidence, DayScore, OptimalWindow};
use crate::geo::{bearing_deg, bearing_label, Point};
use crate::lang::Language;
use crate::score::{Grusindeks, ScoreBreakdown};
use crate::types::RideWindow;
            ...
            .map(|(p, s)| {
                ...
                PointScore {
                    ...
                    bearing_label: if is_center {
                        center_label(lang).to_string()
                    } else {
                        bearing_label(b, lang).to_string()
                    },
                    is_center,
                    score: s,
                }
...
pub struct HourlyDayAggregate {
    ...
    #[serde(default)]
    pub stats: Option<crate::score::WindowStats>,
}
...
mod tests {
    ...
    fn day_aggregate_picks_worst_confidence_across_points() {
        ...
        use crate::types::{HourlyConditions, Resolution};
            ...
            vec![
                (
                    ...
                    compute_day(
                        ...
                        now,
                        Language::Norwegian,
                        crate::daily::BestWindowConfig::default(),
                    ),
                ),
                (
                    other,
    ...
    fn day_aggregate_realigns_window_improvement_against_displayed_mean() {
        ...
        use crate::daily::{compute_day, BestWindowConfig};
